...
```text
รับinputเป็นจำนวนเต็ม โจทย์ต้องการทราบว่า input ที่รับเข้ามาไล่ลงมาถึง 0 มีกี่ค่าที่หาร 10 ลง โดย output ที่ออกมาอยู่ในบรรทัดเดียวและมีช่องว่างกั้น
```
...
```text
เริ่มจากรับinputเข้ามาแล้วคำนวนหาเศษของinputก่อนแล้วเอามาลบออก จากนั้นก็ใช้ loop ให้ไล่จาก n มากยัง 0 โดยขยับมาทีละ -10 จากนั้นก็ print ออกมาโดยหลังจากค่าแต่ละตัวมีช่องว่างกั้น
```
...
```text
ใช้วิธีเดียวกับแผนแรก โดยใช้ loop และ range ในการให้มันทำงานวนซ้ำจนกว่าจะถึงตัวที่กำหนดโดยในข้อนี้ ได้กำหนดให้เริ่มที่  n หยุดก่อน -1 และลงทีละ 10 แล้วก็ใช้ end เพื่อให้ output ออกมาบรรทัดเดียวกันและเว้นด้วยช่องว่าง
```
